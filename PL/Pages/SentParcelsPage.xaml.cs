...
namespace PL.Pages
{
    public partial class SentParcelsPage
    {
        ...
        public SentParcelsPage(BlApi ibl, User user)
        {
            _bl = ibl;
            SentParcels = ibl.GetParcels(p => p.senderId == user.customerId);
            InitializeComponent();
            DataContext = this;

            ParcelViewModels = SentParcels
                .Select(
                    p =>
                        new ParcelViewModel(_bl, p))
                .ToList();
        ...
        private void Panel3_Click(object sender, RoutedEventArgs e)
        {
            ...
            ParcelViewModels = SentParcels
                .Where(p => p.delivered == default && p.collected != default)
                .Select(
                    p =>
                        new ParcelViewModel(_bl, p))
                .ToList();
        ...
        private void Panel4_Click(object sender, RoutedEventArgs e)
        {
            ...
            ParcelViewModels = SentParcels
                .Where(p => p.scheduled == default && p.requested != default)
                .Select(
                    p =>
                        new ParcelViewModel(_bl, p))
                .ToList();
